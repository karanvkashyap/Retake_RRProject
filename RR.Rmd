...
# Introduction
This project seeks to determine whether alcohol consumption has any prognostic significance over average student grades.
We're also interested in duplicating the original study to discover if additional characteristics are significant predictors of student grades. We will not attempt to estimate student grade progression over marking periods because all of the dataset's attributes (except grades) stay consistent between marking periods and are fundamental descriptors of student backgrounds.

The project is organized as follows.

We begin with exploratory data analysis to see if the two tables (with math and Portuguese grades from original dataset) and (with math and english grades from survey dataset) may be combined.
...
Original Dataset
The following are the top ten most important variables influencing student average grades:

Higher- want to pursue higher education (binary: yes or no)
Medu - mother's education (numeric: 0 - no education, 1 - primary education (4th grade), 2 - secondary education or 4 - higher education)
studytime - weekly study time (numeric: 1–2 hours, 2–2–5 hours, 3–5–10 hours, or 4–>10 hours)
schoolsup - further educational assistance (binary: yes or no)
Dalc - alcohol consumption throughout the daytime (numeric: from 1 - very low to 5 - very high)
goout - a social gathering with friends (numeric: from 1 - very low to 5 - very high)
Walc (Weekend alcohol consumption) (numeric: from 1 - very low to 5 - very high)
reason - reason for selecting this school (nominal: proximity to 'home,' school'reputation,' course preference, or 'other')
Fedu - father's education (numeric: 0 - no education, 1 - primary education (4th grade), 2 - secondary education or 4 - higher education)
sex - the gender of the student (binary: 'F' - female or 'M' - male)

These findings show that alcohol intake on both weekdays and weekends is an important predictor of student average grades. The removal of each of these two variables increases the MSE of predictions by 10-20%.
...
In the following sections, I will create a partial dependency plot for each feature in the dataset (ranked by importance). Partial dependency plots represent the marginal influence of a characteristic on the response graphically. In this scenario, the highest performing Random Forest model with 500 trees generates partial dependencies. As the charts show, switching from "extremely low" to "low" alcohol intake on weekdays reduces the projected average grade from 11.22 to 10.88. Similarly, dropping from "extremely low" to "low" alcohol use on weekends reduces the average projected grade from 11.19 to 11.17.

Finally, the effect of the two most relevant factors, "upper" and "medium," on average student grade is as follows:
